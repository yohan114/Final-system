...
import { api, useApi, uploadUrl } from '../api.js';
...
function warrantyLabel(b) {
  if (b.warranty_status === 'expired') return 'Warranty expired';
  if (b.warranty_status === 'expiring') return `Warranty expiring · ${b.warranty_days_left}d`;
  if (b.warranty_status === 'ok') return 'In warranty';
  return null;
}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {reg.data.map((b) => (
              <div key={b.id} className="card overflow-hidden flex flex-col">
                <a href={uploadUrl(b.photo_path)} target="_blank" rel="noreferrer" className="block bg-slate-100 aspect-video overflow-hidden">
                  <img src={uploadUrl(b.photo_path)} alt={`Battery for ${b.vehicle_no}`} className="w-full h-full object-cover hover:scale-105 transition" loading="lazy" />
                </a>
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  {warrantyLabel(b) && (
                    <div className="mt-2 flex items-center gap-2">
                      <Pill className={WARRANTY_STYLE[b.warranty_status]}>{warrantyLabel(b)}</Pill>
                      {b.warranty_end && <span className="text-[11px] text-slate-400">to {b.warranty_end}</span>}
                    </div>
                  )}
...
function BatteryForm({ onClose, onSaved }) {
  ...
  const { busy, err, run } = useSubmit(
    () => api.post('/batteries', {
      vehicle_no: vehicle.trim(), serial_no: serial.trim(), note: note || null, photo,
      installed_date: installedDate || null, warranty_months: warrantyMonths || null, unit_cost: unitCost || null,
    }),
    onSaved, onClose);
    ...
    <Modal title="Add battery" onClose={onClose}>
      <form onSubmit={(e) => { e.preventDefault(); run(() => !vehicle.trim() ? 'Enter the vehicle number' : !serial.trim() ? 'Enter the battery serial number' : !photo ? 'A photo is required' : null); }} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div><label className="label">Installed date</label><input type="date" className="input" value={installedDate} onChange={(e) => setInstalledDate(e.target.value)} /></div>
          <div><label className="label">Warranty (months)</label><input type="number" min="0" className="input" value={warrantyMonths} onChange={(e) => setWarrantyMonths(e.target.value)} placeholder="e.g. 18" /></div>
          <div><label className="label">Unit cost (Rs, optional)</label><input type="number" min="0" step="0.01" className="input" value={unitCost} onChange={(e) => setUnitCost(e.target.value)} placeholder="e.g. 28500" /></div>
        </div>
        <p className="text-[11px] text-slate-400 -mt-2">Warranty drives the expiry alerts. Leave blank if unknown.</p>
        <div>
