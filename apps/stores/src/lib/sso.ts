...
// (base64url(payload).base64url(hmac-sha256), shared secret
// MAINSTORES_SSO_SECRET). Tokens are short-lived and single-use: the jti
// replay guard is in-memory, which holds because one process serves this app.

const seenJti = new Map<string, number>(); // jti -> expiry (ms)

export function verifySsoToken(token: string): { username: string } | null {
  const secret = process.env.MAINSTORES_SSO_SECRET;
  ...
  if (payload.sys !== "mainstores") return null;
