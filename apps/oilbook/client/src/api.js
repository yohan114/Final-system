...
// Build an authenticated URL for a protected /uploads asset. Browsers do not
// send the Authorization header on <img>/<a> loads, so the session token is
// carried as a `t` query param (validated server-side).
export const uploadUrl = (p) => {
  if (!p) return p;
  const t = getToken();
  return t ? `${p}${p.includes('?') ? '&' : '?'}t=${encodeURIComponent(t)}` : p;
};
