...
- ✅ **Battery lifecycle in S4** *(shipped)* — the battery register now records installed date + warranty months + unit cost, derives a warranty status (ok / expiring within 60 days / expired), shows it on the card, and exposes `GET /api/batteries/alerts` (expired + expiring, worst-first). Verified via the API (8/8). The new `unit_cost` field also **unblocks the battery cost category** below.
- ⏸ **Battery / store cost categories** — S3 store issues remain free-text-only (would land unattributed); battery cost is now unblocked (S4 `unit_cost` exists) — a future increment can add a battery cost feed to `/api/portal/costs`.
