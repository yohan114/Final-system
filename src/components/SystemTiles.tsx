...
export interface TileSystem {
  ...
  icon: string;
  openUrl: string;
  // Where the "Open system" button actually navigates: /launch/<key> when SSO
  // is configured (arrive signed in), else the system's plain address.
  launchUrl?: string;
}
        ...
        {systems.map((sys) => {
          ...
          return (
            <div key={sys.key} className="bg-card border border-card-border rounded-2xl p-5 flex flex-col gap-4">
              ...
              <a
                href={sys.launchUrl || sys.openUrl}
                target="_blank"
                rel="noreferrer"
